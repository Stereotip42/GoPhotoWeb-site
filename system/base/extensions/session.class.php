...
class SessionExtension {
     ...
     *
     * @access public
     * @param string $crossdomain cookie would be access in all subdomains this domain
     * @return void
     */
    public function __construct($crossdomain = NULL) {
        /**
         * Init session and create 'session' array
         */
        if (!isset($_SESSION)) {
            if (NULL != $crossdomain) {
                session_set_cookie_params('Session', '/', '.'.$crossdomain);
            }
            session_start();
