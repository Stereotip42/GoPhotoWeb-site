...
class Db {
     ...
     */
    private static $_statistic = array('time' => 0, 'runtime' => 0, 'total' => 0);
    /**
    ...
    public static function connect() {
        try {
            self::$_pdo = new PDO(Config::get('database/driver').':host='.Config::get('database/host').';dbname='.Config::get('database/db'), Config::get('database/user'), Config::get('database/pass'));
     ...
     * @return void
     */
    public static function setIdentPrefix($prefix) {
        self::$_prefix = $prefix;
    }
    ...
    /**
     * Return db profile (queries count and runtime)
     *
     * @access public 
     * @return array
     */
    ...
    private static function queryRuntime($action) {
        switch ($action) {
            case 'start':
                self::$_statistic['time'] = microtime(true);
                break;
            case 'finish':
                self::$_statistic['runtime'] += round(microtime(true) - self::$_statistic['time'], 4);
                self::$_statistic['total']++;
                self::$_statistic['time'] = 0.00;
                break;
        }
    }
