...
class View {
    ...
    public function __call($name, $arguments) {
        ...
        if (!isset($this->_helpers[$helper]) && file_exists(Config::get('path/helpers').$helper.'.class.php')) {
            require_once(Config::get('path/helpers').$helper.'.class.php');
            $class = ucfirst($helper).'Helper';
            $this->_helpers[$helper] = new $class;
            return $this->_helpers[$helper]->execute($arguments);
        }
        elseif (isset($this->_helpers[$helper])) {
            return $this->_helpers[$helper]->execute($arguments);
        }
        else {
            throw new CException("Helper file {$helper}.class.php not found", 2);
        }
    }
